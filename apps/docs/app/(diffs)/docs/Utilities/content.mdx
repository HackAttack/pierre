...
`'zed'` for Highlights. Existing two-argument calls keep working. The loader
accepts a TextMate theme, a Zed-compatible `Theme` or `ThemeFamily`, or a
portable `DiffsTheme`, directly or as a module's `default` export.
...
and Zed loaders. Register a portable `DiffsTheme` for both types to use its
palettes across backends. `registerCustomCSSVariableTheme` registers both types
automatically.

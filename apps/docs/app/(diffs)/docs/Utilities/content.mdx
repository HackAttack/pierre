...
Register a lazy loader for a TextMate theme, a Zed-compatible Highlights `Theme`
or `ThemeFamily`, or a portable `DiffsTheme`. The loader can return the theme
directly or as a module's `default` export.
...
Register one loader per name. Raw TextMate themes require Shiki, and raw Zed
themes require Highlights. To support every backend with one theme, use
`createCSSVariablesTheme` or a `DiffsTheme` containing both a normalized
`textmate` palette and a `zed` palette.

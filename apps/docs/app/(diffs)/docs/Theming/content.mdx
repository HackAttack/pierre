...
`registerCustomTheme` accepts TextMate themes for Shiki, Zed-compatible
Highlights themes and theme families, and portable `DiffsTheme` objects. A Zed
family uses its first theme; return another member from the loader to select it.
The registered name replaces the Zed theme's display name.

Use `preferredHighlighter: 'highlights'` with a Zed theme. Register one loader
per name. For a theme that works across backends, use `createCSSVariablesTheme`
or a `DiffsTheme` with both syntax palettes.
